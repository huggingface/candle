/**
 * @file moe_q4k_imma_m8_chunks.cu
 *
 * Chunked variant of moe_q4k_imma_m8. Host pre-computes a per-block
 * (pair_start, expert) array so that every 8-pair block has a SINGLE
 * expert — no boundary-block multi-pass needed in the kernel.
 *
 * Saves the ~4-15% perf cost the in-kernel multi-pass incurred on
 * boundary blocks (which is ~39% of blocks at gemma4:26b's typical
 * routing density with 128 experts × 16 pairs/expert).
 *
 * Grid:
 *   gridDim.x = ceil(N / 16)
 *   gridDim.y = num_chunks                ← host-supplied
 *   gridDim.z = 1
 * blockDim = 32 (1 warp)
 *
 * Each block reads chunk_pair_start[blockIdx.y] for its pair_base and
 * chunk_expert[blockIdx.y] for its weight expert.
 */
#include "gguf.cuh"
#include <cuda.h>
...
namespace moe_q4k_imma_m8_chunks_ns {

#define IMC_K_SUPER 256
#define IMC_M       16
#define IMC_N        8
...
static_assert(sizeof(block_q4_K_imc) == 144, "block_q4_K_imc size");
static_assert(sizeof(block_q8_1_imc) == 36,  "block_q8_1_imc size");

__device__ __forceinline__ void imc_get_scale_min_k4(
...
extern "C" __global__ void moe_q4k_imma_m8_chunks_kernel(
    ...
    const int m_base = m_tile * IMC_M;
    if (m_base >= N) return;

    const int lane = threadIdx.x;
    const int g    = lane >> 2;
    const int tj   = lane & 3;
    const int num_super = K / IMC_K_SUPER;
    const int two_n = N << 1;
    ...
    const block_q8_1_imc * y_expert = nullptr;
    if (v_pair_e) {
        const int real_token = my_pair_tok / topk;
        y_expert = (const block_q8_1_imc *) inputs_q81
    ...
    for (int isb = 0; isb < num_super; ++isb) {
        const block_q4_K_imc * gwa = va ? w_expert + (size_t)row_a    * num_super + isb : nullptr;
        const block_q4_K_imc * gwb = vb ? w_expert + (size_t)row_b    * num_super + isb : nullptr;
        const block_q4_K_imc * uwa = va ? w_expert + (size_t)row_a_up * num_super + isb : nullptr;
        const block_q4_K_imc * uwb = vb ? w_expert + (size_t)row_b_up * num_super + isb : nullptr;
        ...
        for (int s = 0; s < 8; ++s) {
            uint8_t sc, m;
            if (gwa) { imc_get_scale_min_k4(s, gwa->scales, sc, m); g_da_a[s] = g_dall_a * (float)sc; g_dm_a[s] = g_dmin_a * (float)m; }
            else     { g_da_a[s] = 0.f; g_dm_a[s] = 0.f; }
            if (gwb) { imc_get_scale_min_k4(s, gwb->scales, sc, m); g_da_b[s] = g_dall_b * (float)sc; g_dm_b[s] = g_dmin_b * (float)m; }
            else     { g_da_b[s] = 0.f; g_dm_b[s] = 0.f; }
            if (uwa) { imc_get_scale_min_k4(s, uwa->scales, sc, m); u_da_a[s] = u_dall_a * (float)sc; u_dm_a[s] = u_dmin_a * (float)m; }
            else     { u_da_a[s] = 0.f; u_dm_a[s] = 0.f; }
            if (uwb) { imc_get_scale_min_k4(s, uwb->scales, sc, m); u_da_b[s] = u_dall_b * (float)sc; u_dm_b[s] = u_dmin_b * (float)m; }
        ...
        for (int s = 0; s < 8; ++s) {
            ...
            const block_q8_1_imc * yb_my = y_expert ? y_expert + isb * 8 + s : nullptr;
    ...
    // do_write helper doesn't re-read expert_ids / sorted_token_ids
    // from global memory four times per lane.
...
extern "C" void moe_q4k_imma_m8_chunks_gate_up(
