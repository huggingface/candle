/**
 * @file moe_q4k_imma_m8_2w.cu
 *
 * 2-warp IMMA M=8: each block has 2 INDEPENDENT warps handling
 * different m_tiles. Same shared (sorted pair) tile, different
 * 16-weight-row groups. Block produces 32 weight rows × 8 pairs.
 *
 * Independent warps means no __syncthreads / shared mem needed.
 * Pure register-level kernel, just packed 2 to a block to reduce
 * grid-X by 2× → less per-block launch overhead.
 */
#include "gguf.cuh"
#include <cuda.h>
...
namespace moe_q4k_imma_m8_2w_ns {

#define I2W_K_SUPER 256
#define I2W_M_PER_WARP 16
#define I2W_N           8
#define I2W_NWARPS      2
#define I2W_M_PER_BLOCK (I2W_M_PER_WARP * I2W_NWARPS)
...
static_assert(sizeof(block_q4_K_2w) == 144, "block_q4_K_2w size");
static_assert(sizeof(block_q8_1_2w) == 36,  "block_q8_1_2w size");

__device__ __forceinline__ void w2_get_scale_min_k4(
...
extern "C" __global__ void moe_q4k_imma_m8_2w_kernel(
    ...
    using namespace moe_q4k_imma_m8_2w_ns;

    const int n_tile = blockIdx.y;
    const int pair_base = n_tile * I2W_N;
    if (pair_base >= size_m) return;

    const int m_block_tile = blockIdx.x;
    const int m_block_base = m_block_tile * I2W_M_PER_BLOCK;
    if (m_block_base >= N) return;

    const int warp_id = threadIdx.y;
    const int m_base = m_block_base + warp_id * I2W_M_PER_WARP;
    if (m_base >= N) return;

    const int lane = threadIdx.x;
    const int g    = lane >> 2;
    const int tj   = lane & 3;
    const int num_super = K / I2W_K_SUPER;
    const int two_n = N << 1;
    ...
    const block_q4_K_2w * w_expert =
        (const block_q4_K_2w *) gate_up_w
        + (size_t)block_expert * (size_t)two_n * num_super;

    const block_q8_1_2w * y_expert = nullptr;
    if (v_pair_e) {
        const int real_token = my_pair_tok / topk;
        y_expert = (const block_q8_1_2w *) inputs_q81
    ...
    for (int isb = 0; isb < num_super; ++isb) {
        const block_q4_K_2w * gwa = va ? w_expert + (size_t)row_a    * num_super + isb : nullptr;
        const block_q4_K_2w * gwb = vb ? w_expert + (size_t)row_b    * num_super + isb : nullptr;
        const block_q4_K_2w * uwa = va ? w_expert + (size_t)row_a_up * num_super + isb : nullptr;
        const block_q4_K_2w * uwb = vb ? w_expert + (size_t)row_b_up * num_super + isb : nullptr;
        ...
        for (int s = 0; s < 8; ++s) {
            uint8_t sc, m;
            if (gwa) { w2_get_scale_min_k4(s, gwa->scales, sc, m); g_da_a[s] = g_dall_a * (float)sc; g_dm_a[s] = g_dmin_a * (float)m; }
            else     { g_da_a[s] = 0.f; g_dm_a[s] = 0.f; }
            if (gwb) { w2_get_scale_min_k4(s, gwb->scales, sc, m); g_da_b[s] = g_dall_b * (float)sc; g_dm_b[s] = g_dmin_b * (float)m; }
            else     { g_da_b[s] = 0.f; g_dm_b[s] = 0.f; }
            if (uwa) { w2_get_scale_min_k4(s, uwa->scales, sc, m); u_da_a[s] = u_dall_a * (float)sc; u_dm_a[s] = u_dmin_a * (float)m; }
            else     { u_da_a[s] = 0.f; u_dm_a[s] = 0.f; }
            if (uwb) { w2_get_scale_min_k4(s, uwb->scales, sc, m); u_da_b[s] = u_dall_b * (float)sc; u_dm_b[s] = u_dmin_b * (float)m; }
        ...
        for (int s = 0; s < 8; ++s) {
            ...
            const block_q8_1_2w * yb_my = y_expert ? y_expert + isb * 8 + s : nullptr;
...
extern "C" void moe_q4k_imma_m8_2w_gate_up(
    ...
    using namespace moe_q4k_imma_m8_2w_ns;
    dim3 grid((N      + I2W_M_PER_BLOCK - 1) / I2W_M_PER_BLOCK,
              (size_m + I2W_N           - 1) / I2W_N,
              1);
    dim3 blk(32, I2W_NWARPS, 1);
    moe_q4k_imma_m8_2w_kernel<<<grid, blk, 0, stream>>>(
