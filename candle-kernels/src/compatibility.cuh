...
__device__ __forceinline__ __half atomicAdd(__half* address, __half val) {
    ...
    do {
        ...
        const unsigned short old_bits =
            upper ? (unsigned short)(old >> 16) : (unsigned short)(old & 0xffff);
        const __half current = __ushort_as_half(old_bits);
        const __half sum = __float2half(__half2float(current) + __half2float(val));
        const unsigned int sum_bits = (unsigned int)__half_as_ushort(sum);
        const unsigned int updated = upper
...
__device__ __forceinline__ __nv_bfloat16 atomicAdd(__nv_bfloat16* address, __nv_bfloat16 val) {
    ...
    do {
        ...
        const unsigned short old_bits =
            upper ? (unsigned short)(old >> 16) : (unsigned short)(old & 0xffff);
        const __nv_bfloat16 current = __ushort_as_bfloat16(old_bits);
        const __nv_bfloat16 sum =
            __float2bfloat16(__bfloat162float(current) + __bfloat162float(val));
        const unsigned int sum_bits = (unsigned int)__bfloat16_as_ushort(sum);
        const unsigned int updated = upper
            ? ((old & 0x0000ffffu) | (sum_bits << 16))
            : ((old & 0xffff0000u) | sum_bits);
        old = atomicCAS(address_as_ui, assumed, updated);
