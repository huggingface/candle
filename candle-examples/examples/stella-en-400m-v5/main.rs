...
use candle_transformers::models::stella_en_v5_400m::{
    Config,
    EmbedDim as StellaEmbedDim,
    EmbeddingModel,
};
...
impl Embedding {
    ...
    fn encode(&mut self, task: EncodeTask, text: Option<String>) -> Result<()> {
        ...
        if let Some(text) = text {
            ...
            let mut qry_encoded = self.tokenizer.encode_batch(qry, true).map_err(|e| anyhow!(e))?;

            let mut docs_encoded = self.tokenizer
            ...
            let answer = self.similarity(&qry_embed, &doc_embed);
            println!("Similarity========\n {}", answer.unwrap());
    ...
    /// Computes the cosine similarity between two tensors of embeddings
    /// Similar to sentence-transformers' similarity() function
    pub fn similarity(&self, embeddings1: &Tensor, embeddings2: &Tensor) -> Result<Tensor> {
        // Normalize the embeddings (L2 norm)
        let norm1 = embeddings1.broadcast_div(&embeddings1.sqr()?.sum_keepdim(1)?.sqrt()?)?;
        let norm2 = embeddings2.broadcast_div(&embeddings2.sqr()?.sum_keepdim(1)?.sqrt()?)?;

        // Compute cosine similarity: dot product of normalized vectors
        Ok(norm1.matmul(&norm2.t()?)?)
    }

    fn _encode_single_doc(&mut self) -> Result<Tensor> {
        // Example document text
        let doc =
            "There are many effective ways to reduce stress. Some common techniques include deep breathing, meditation, and physical activity. Engaging in hobbies, spending time in nature, and connecting with loved ones can also help alleviate stress. Additionally, setting boundaries, practicing self-care, and learning to say no can prevent stress from building up.".to_string();

        // Encode the document
        let encoding = self.tokenizer.encode(doc, true).map_err(|e| anyhow!(e))?;

        // Create input tensors
        let shape = (1, encoding.len());
        let ids = Tensor::from_slice(encoding.get_ids(), shape, &self.device)?;
        let mask = Tensor::from_slice(encoding.get_attention_mask(), shape, &self.device)?;

        // Get embeddings and print intermediate values
        let embeddings = self.model.forward_norm(&ids, &mask)?;

        // Print the shape and first few values
        println!("Document embedding shape: {:?}", embeddings.shape());
        println!("First few values: {:?}", embeddings.i(0)?.narrow(0, 0, 3)?.to_vec1::<f32>()?);

        // Return normalized embeddings
        let norm_embeddings = embeddings.broadcast_div(&embeddings.sqr()?.sum_keepdim(1)?.sqrt()?)?;
...
fn main() -> Result<()> {
    ...
    let repo = api.repo(Repo::model("dunzhang/stella_en_400M_v5".to_string()));
    ...
    let base_vb = unsafe {
        VarBuilder::from_mmaped_safetensors(&base_weight_files, dtype, &device)?
    };
    // Embedding layer is always built on F32 for accuracy
    let embed_vb = unsafe {
        VarBuilder::from_mmaped_safetensors(&embed_weight_files, DType::F32, &device)?
    };

    let model = EmbeddingModel::new(&Config::new_400m(embed_dim.embed_dim()), base_vb, embed_vb)?;
    ...
    // let _doc_esmbeddings = embedding._encode_single_doc()?;
