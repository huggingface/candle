...
pub fn load_image448_openai_norm<P: AsRef<std::path::Path>, B: BackendStorage>(
    p: P,
    device: &B::Device,
) -> Result<Tensor<B>> {
    ...
    let data = Tensor::from_vec(data, (448, 448, 3), device)?.permute((2, 0, 1))?;
...
fn run<B: BackendStorage>(args: Args) -> anyhow::Result<()> {
    ...
    let image: Tensor<B> = load_image448_openai_norm(args.image, &device)?;
