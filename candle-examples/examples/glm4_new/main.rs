...
use candle_transformers::models::glm4_new::*;
use clap::Parser;
...
impl TextGeneration {
    ...
    fn new(model: ModelForCausalLM, tokenizer: Tokenizer, args: Args, device: &Device) -> Self {
    ...
    fn run(&mut self) -> anyhow::Result<()> {
        ...
        let eos_token = match self.tokenizer.get_vocab(true).get("<|user|>") {
            Some(token) => *token,
...
fn main() -> anyhow::Result<()> {
    ...
    let mut pipeline = TextGeneration::new(model, tokenizer, args, &device);
    pipeline.run()?;
    Ok(())
}
