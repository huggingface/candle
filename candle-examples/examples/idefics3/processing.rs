...
impl Idefics3ImageProcessor {
    ...
    fn pad_image(
        ...
        if data_format == "channels_first" {
            ...
            padded_image = padded_image
                .slice_assign(&[0..3, 0..input_height, 0..input_width], &transposed_image).unwrap();
            pixel_mask = pixel_mask.slice_assign(
                &[0..input_height, 0..input_width],
                &Tensor::ones((input_height, input_width), DType::I64, device)?,
            )?;
        } else {
            padded_image = padded_image.slice_assign(&[0..3, 0..input_height, 0..input_width], &image).unwrap();
            pixel_mask = pixel_mask.slice_assign(
                &[0..input_height, 0..input_width],
                &Tensor::ones((input_height, input_width), DType::I64, device)?,
            )?;
        ...
        for (batch_idx, batch) in images.iter().enumerate() {
            for (sample_idx, image) in batch.iter().enumerate() {
                let (padded_image, pixel_mask) =
                    self.pad_image(image, output_size, data_format, device).unwrap();
    ...
    fn _preprocess_one_image(
        ...
        let (frames, n_rows, n_cols) = if self.do_image_splitting {
            ...
            let frame = Tensor::from_vec(frame, (vision_encoder_image.height() as usize, vision_encoder_image.width() as usize, 3), device)?;
            (vec![frame], 1, 1)
        };
...
fn normalize(image: &Tensor, mean: &Tensor, std: &Tensor) -> Result<Tensor, anyhow::Error> {
    let normalized_image = image.to_dtype(DType::F32)?.broadcast_sub(mean)?.broadcast_div(std)?;
    Ok(normalized_image)
}
...
mod tests {
    ...
    fn image_resize_test() {
        ...
        let image_mean = Tensor::from_vec(image_mean.clone(), (1, 1, image_mean.len()), &Device::Cpu)
            .unwrap();
        let image_std =
    ...
    fn test_idefics3_processor() {
        ...
        let (input_ids, attention_mask, preprocessed_images, preprocessed_masks) = processor.preprocess(&[image], &Device::Cpu).unwrap();
        println!("Input IDs: {:?}", input_ids);
        println!("Attention Mask: {:?}", attention_mask);
        println!("Preprocessed Images: {}", preprocessed_images);
        println!("Preprocessed Masks: {:?}", preprocessed_masks.unwrap().sum_all());
    }
}
