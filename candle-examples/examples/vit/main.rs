...
use candle::{BackendDevice, BackendStorage, DType, IndexOp, D};
...
fn run<B: BackendStorage>(args: Args) -> anyhow::Result<()> {
    ...
    let image = candle_examples::imagenet::load_image224(args.image, &device)?;
    ...
    let model: vit::Model<B> = vit::Model::new(&vit::Config::vit_base_patch16_224(), 1000, vb)?;
    println!("model built");
