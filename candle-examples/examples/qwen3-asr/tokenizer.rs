//! Tokenizer loading for Qwen3-ASR (BPE: vocab.json + merges.txt).
//!
//! Adapted from PR #3376 (lumosimmo).
...
pub fn build_qwen_bpe_tokenizer(
    ...
    let mut vocab_map: HashMap<String, u32> = serde_json::from_str(&vocab_content)
        .with_context(|| format!("failed to parse vocab.json from {vocab_path:?}"))?;

    let added_tokens = load_added_tokens_from_config(config_path)?;
    apply_added_tokens_to_vocab(&mut vocab_map, &added_tokens)?;

    let vocab: tokenizers::models::bpe::Vocab = vocab_map.into_iter().collect();

    let merges_content = std::fs::read_to_string(merges_path)
        .with_context(|| format!("failed to read merges.txt from {merges_path:?}"))?;
    let merges: tokenizers::models::bpe::Merges = merges_content
        .lines()
        .skip(1)
    ...
    let mut tokenizer: HfTokenizer = tokenizer.into();
