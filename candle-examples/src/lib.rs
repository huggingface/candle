...
pub mod wav;
use candle::utils::{cuda_is_available, metal_is_available, rocm_is_available};
...
pub fn device(cpu: bool) -> Result<Device> {
    if cpu {
    ...
    } else if rocm_is_available() {
        Ok(Device::new_rocm(0)?)
        ...
        {
            println!("Running on CPU, to run on GPU, build this example with `--features cuda` or `--features rocm`");
        }
        Ok(Device::Cpu)
    }
}
