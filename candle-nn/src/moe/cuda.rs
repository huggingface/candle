//! The CUDA fused-expert GEMMs, split out of `super` so the dispatcher there
//! stays readable. Both bind kernels from `candle-kernels/src/moe/`, compiled
//! into the `libmoe.a` static library; see [`super::moe_gemm_gguf`] for why the
//! ROCm side cannot use them.
