use std::{collections::HashMap, fs::File, future::IntoFuture};
...
pub fn bench_function_single<F : Fn() -> T, T>(func : F) -> web_time::Duration{
...
pub fn bench_function_max_time<F : Fn() -> T, T>(id : &str, func : F, max_time : Duration) -> Measurement{
    ...
    for _ in 0..100{
        durations.push(bench_function_single(&func));
    
        if Instant::now() - start > max_time{
            break;
        }
    }
    let durations_f64 : Vec<f64> = durations.iter().map(|c| c.as_secs_f64()).collect();
    let count = durations.len();

    let d_min = durations.iter().min().unwrap();
    let d_max = durations.iter().max().unwrap();
    ...
    let result = Measurement{ 
        label: id.to_owned(), 
        mean: d_mean, 
        min: d_min.as_secs_f64(),
        max: d_max.as_secs_f64(), 
        std: d_std,
        count : count as u32 };

    warn!("time: [{:?} {:?} {:?}] +-{:?} ({count} iterations)", d_min, Duration::from_secs_f64(d_mean) ,d_max, Duration::from_secs_f64(d_std));
    return result;
}
