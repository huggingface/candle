...
impl MmapedSafetensors {
    ...
    pub fn load(&self, name: &str, dev: &Device) -> GenericResult<Tensor> {
      ...
      Ok(Tensor::from_raw_buffer(tensor_view.data(), dtype, tensor_view.shape(), dev)?)
    }
...
pub async fn var_builder_from_opfs_safetensors<P: AsRef<Path>>(p: P, dtype: DType, dev: &Device) -> GenericResult<VarBuilder> {
  let tensors = MmapedSafetensors::new(p).await?;

  Ok(VarBuilder::from_backend(Box::new(tensors), dtype, dev.clone()))
}