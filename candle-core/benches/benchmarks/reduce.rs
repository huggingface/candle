...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    for device in handler.devices {
        ...
        run_reduce_large(c, &device, (lo, up), true);
...
fn run_reduce_large<T: candle_core::FloatDType>(
    c: &mut Criterion,
    device: &Device,
    (lo, up): (T, T),
    strided: bool,
