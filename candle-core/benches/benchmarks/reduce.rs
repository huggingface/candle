...
fn run_sum<B: BackendStorage>(a: &Tensor<B>) {
    a.sum_keepdim(2).unwrap();
}
fn run_arg_min<B: BackendStorage>(a: &Tensor<B>) {
...
fn run_reduce<B, D, T>(c: &mut Criterion, device: &D, (lo, up): (T, T), strided: bool)
where
...
fn run_arg_reduce<B, D, T>(c: &mut Criterion, device: &D, (lo, up): (T, T), strided: bool)
where
    B: BackendStorage<Device = D>,
    D: BenchDevice<B>,
    T: candle_core::FloatDType,
