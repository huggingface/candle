...
fn run<B: BackendStorage>(a: &Tensor<B>, b: &Tensor<B>, c: &Tensor<B>) {
...
fn run_where_cond_benchmark<B: BackendStorage<Device = D>, D: BenchDevice<B>>(
    c: &mut Criterion,
    device: &D,
    dtype: DType,
    name: &str,
) {
    let tensor: Tensor<B> = Tensor::from_slice(DATA.as_slice(), (B, M, K), device).unwrap();
