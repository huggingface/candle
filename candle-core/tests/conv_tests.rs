...
/* conv_transpose1d tests with stride > 1 and padding, verified against PyTorch:

import torch
torch.manual_seed(42)

# Test 1: stride=2, padding=0
t = torch.randn(1, 4, 8)
w = torch.randn(4, 2, 4)
res = torch.nn.functional.conv_transpose1d(t, w, stride=2)
# shape: [1, 2, 18]

# Test 2: stride=4, padding=2 (VAE-like: kernel=2*stride, padding=stride//2)
torch.manual_seed(42)
t2 = torch.randn(1, 8, 16)
w2 = torch.randn(8, 4, 8)
res2 = torch.nn.functional.conv_transpose1d(t2, w2, stride=4, padding=2)
# shape: [1, 4, 64]

# Test 3: stride=10, padding=5 (Oobleck decoder pattern)
torch.manual_seed(42)
t3 = torch.randn(1, 4, 6)
w3 = torch.randn(4, 2, 20)
res3 = torch.nn.functional.conv_transpose1d(t3, w3, stride=10, padding=5)
# shape: [1, 2, 60]
*/
fn conv_transpose1d(dev: &Device) -> Result<()> {
    // Test 1: stride=2, no padding
    let t = Tensor::new(
        &[
    ...
    let w = Tensor::new(
        &[
            -1.3847f32, -0.8712, -0.2234, 1.7174, 0.3189, -0.4245, 0.3057, -0.7746, -1.5576,
            0.9956, -0.8798, -0.6011, -1.2742, 2.1228, -1.2347, -0.4879, -0.9138, -0.6581, 0.0780,
            0.5258, -0.4880, 1.1914, -0.8140, -0.7360, -1.4032, 0.0360, -0.0635, 0.6756, -0.0978,
            1.8446, -1.1845, 1.3835,
        ],
        dev,
    )?
    .reshape((4, 2, 4))?;
    let res = t.conv_transpose1d(&w, 0, 0, 2, 1, 1)?;
    assert_eq!(res.dims(), [1, 2, 18]);
    let expected = [
        -4.7924f32, -3.4625, -6.0225, 5.9866, -2.9155, 1.5289, 2.8936, 2.1294, 2.8669, -2.4644,
        1.9812, 0.8579, 1.7278, -2.4527, -0.9435, 1.7214, -0.2358, -1.7495, 0.6462, 1.9014,
        -3.0097, 4.5051, -2.0163, -0.7274, 1.2136, 1.6045, 0.6868, 0.6753, 1.7864, 0.7654, -0.0142,
        -0.6304, -1.8052, 5.3591, -3.8185, 0.8237,
    ];
    let res_vec = test_utils::to_vec1_round(&res.flatten_all()?, 4)?;
    for (a, b) in res_vec.iter().zip(expected.iter()) {
        assert!(
            (a - b).abs() < 0.02,
            "stride=2: mismatch {a} vs {b} (tol 0.02)"
        );
    }

    // Test 3: stride=10, padding=5 (Oobleck VAE decoder pattern: kernel=2*stride, pad=stride/2)
    // torch.manual_seed(42); t3 = torch.randn(1,4,6); w3 = torch.randn(4,2,20)
    let t3 = Tensor::new(
        &[
            1.9269f32, 1.4873, 0.9007, -2.1055, 0.6784, -1.2345, -0.0431, -1.6047, 0.3559, -0.6866,
            -0.4934, 0.2415, -1.1109, 0.0915, -2.3169, -0.2168, -0.3097, -0.3957, 0.8034, -0.6216,
            -0.592, -0.0631, -0.8286, 0.3309,
        ],
        dev,
    )?
    .reshape((1, 4, 6))?;
    let w3 = Tensor::new(
        &[
            0.0349f32, 0.3211, 1.5736, -0.8455, 1.3123, 0.6872, -1.0892, -0.3553, -1.4181, 0.8963,
            0.0499, 2.2667, 1.179, -0.4345, -1.3864, -1.2862, -0.8371, -0.9224, 1.8113, 0.1606,
            0.3672, 0.1754, 1.3852, -0.4459, -1.2024, 0.7078, -1.0759, 0.5357, 1.1754, 0.5612,
            -0.4527, -0.7718, 0.1453, 0.2311, 0.0087, -0.1423, 0.1971, -1.1441, 0.3383, 1.6992,
            2.814, 0.3598, -0.0898, 0.4584, -0.5644, 1.0563, -1.4692, 1.4332, 0.7281, -0.7106,
            -0.6021, 0.9604, 0.4048, -1.3543, -0.4976, 0.4747, -0.1976, 1.2683, 1.2243, 0.0981,
            1.7423, -1.3527, 0.2191, 0.5526, -0.6788, 0.5743, 0.1877, -0.3576, -0.3165, 0.5886,
            -0.8905, 0.4098, -0.9864, 0.1233, 0.3499, 0.6173, -0.1693, 0.2332, 4.0356, 1.2795,
            1.0311, -0.7048, 1.0131, -0.3308, 0.5177, 0.3878, -0.5797, -0.1691, -0.5733, 0.5069,
            -0.4752, -0.492, 0.2704, -0.5628, 0.6793, 0.4405, -0.3609, -0.0606, 0.0733, 0.8187,
            1.4805, 0.3449, -1.4241, -0.1163, 0.2176, -0.0467, -1.4335, -0.5665, -0.4253, 0.2625,
            -1.4391, 0.5214, 1.0414, -0.3997, -2.2933, 0.4976, -0.4257, -1.3371, -0.1933, 0.6526,
            -0.3063, -0.3302, -0.9808, 0.1947, -1.6535, 0.6814, 1.4611, -0.3098, 0.9633, -0.3095,
            0.5712, 1.1179, -1.2956, 0.0503, -0.5855, -0.39, 0.9812, -0.6401, -0.4908, 0.208,
            -1.1586, -0.9637, -0.375, 0.8033, 0.7165, 1.5335, -1.451, -0.7861, -0.9563, -1.2476,
            -0.7499, -0.5922, -1.5326, -0.7251, 0.4664, 0.6667, -0.0439, 0.2368, -0.7061, -0.7169,
        ],
        dev,
    )?
    .reshape((4, 2, 20))?;
    let res3 = t3.conv_transpose1d(&w3, 5, 0, 10, 1, 1)?;
    assert_eq!(res3.dims(), [1, 2, 60]);
    let expected3 = [
        1.3953f32, -0.2176, -0.8073, -1.353, 0.9459, -3.07, 5.8121, 4.1, -2.2576, 0.0577, -4.3627,
        -0.639, -4.93, -0.9669, 2.275, -0.533, 3.3357, 1.5526, 1.4971, -0.0767, -2.6989, -2.5967,
        -2.2495, 0.7811, -0.4097, -1.6163, 2.1121, -2.0629, 1.9258, -5.0367, -4.0786, 2.7666,
        -0.0399, 4.7374, -3.3299, -1.0549, -4.8637, -1.1264, 1.1049, 5.5288, 1.5717, 0.8694,
        0.4857, -6.5812, 0.4621, 0.1319, 0.1502, -1.1, 1.8559, -2.9287, -1.4413, 0.5311, 0.0468,
        3.481, -1.9465, 0.1701, -2.0018, -1.8934, 0.4486, 1.1288, 2.623, -1.6545, 1.0453, 1.9826,
        -0.238, -1.2317, 0.5023, -0.2543, -1.7585, 1.7993, -1.1442, -0.306, 1.2683, 2.9371, 2.6077,
        -0.7035, -1.9417, 7.6942, 0.149, -3.3013, -1.5288, 3.8312, -0.2123, -3.0539, 1.8247,
        0.8362, -0.8613, -4.4601, 2.1413, 8.0751, -3.4273, 3.6683, 1.3005, 0.5019, -0.6659, 1.815,
        2.7459, 1.8256, -1.6434, -0.9326, -1.3334, 0.6201, 3.8905, -1.5192, -3.5097, 0.1974,
        -1.3952, 0.3149, 1.8162, 1.6287, -1.3168, 1.8461, -1.4569, -2.7922, -0.154, 0.6652, 0.6495,
        -1.3369, -0.3372, 1.1356,
    ];
    let res3_vec = test_utils::to_vec1_round(&res3.flatten_all()?, 4)?;
    for (a, b) in res3_vec.iter().zip(expected3.iter()) {
        assert!(
            (a - b).abs() < 0.02,
            "stride=10,pad=5: mismatch {a} vs {b} (tol 0.02)"
        );
    }
...
test_device!(
    ...
    conv_transpose1d_gpu,
    conv_transpose1d_metal
);
