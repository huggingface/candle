...
use candle_macros::{register_quantized_types, QuantizedType};

/// Marker trait for quantized data types
/// 
/// Types implementing this trait must also provide these inherent methods:
/// 
...
/// - `fn matmul(lhs_f32: &[f32], lhs_shape: &[usize], rhs_data: &[u8], rhs_shape: &[usize]) -> Result<Vec<f32>>`
/// 
/// **Optional CUDA operations (feature = "cuda"):**
/// - `fn dequantize_cuda(data: &CudaSlice<u8>, output: &mut CudaSlice<f32>) -> Result<()>`
/// - `fn matmul_cuda(lhs: &CudaSlice<u8>, lhs_shape: &[usize], rhs: &CudaSlice<u8>, rhs_shape: &[usize]) -> Result<CudaSlice<u8>>`
/// 
/// **Optional Metal operations (feature = "metal"):**
/// - `fn dequantize_metal(data: &metal::Buffer, output: &mut metal::Buffer) -> Result<()>`
/// - `fn matmul_metal(lhs: &metal::Buffer, lhs_shape: &[usize], rhs: &metal::Buffer, rhs_shape: &[usize]) -> Result<metal::Buffer>`
/// 
/// Use `#[derive(QuantizedType)]` to automatically implement this trait with NAME and SIZE_IN_BYTES constants.
pub trait QuantizedType: Send + Sync + 'static {
    /// Name identifier for this quantized type (e.g., "q4_0", "q8_0")
    const NAME: &'static str;
    
    /// Size of one element in bytes (can be fractional, represented as numerator/denominator)
    /// For example: 4-bit = 1 byte per 2 elements, 8-bit = 1 byte per element
    const SIZE_IN_BYTES: usize;
}

#[derive(QuantizedType)] 
#[quantized(name = "q4_0", size_in_bytes = 1)]
pub struct DummyQuantizedType;

impl DummyQuantizedType {
    ...
    pub fn storage_size_in_bytes(_num_elements: usize) -> usize {
        0
    }
...
impl DType {
    ...
    pub fn as_str(&self) -> &'static str {
        match self {
            ...
            Self::F64 => "f64",
            Self::F8E4M3 => "f8_e4m3",
            Self::Quantized(qdtype) => qdtype.name(),
        }
    }
    ...
    pub fn size_in_bytes(&self) -> usize {
        match self {
            ...
            Self::F32 => 4,
            Self::F64 => 8,
            Self::Quantized(qdtype) => qdtype.size_in_bytes(),
        }
    }
