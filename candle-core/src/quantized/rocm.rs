// Created by: TEAM-502 (Phase 2)
// ROCm quantization support - Full CUDA parity
// CUDA parity: quantized/cuda.rs (complete mirror)
// Updated by: TEAM-502 (Phase 1-3: Type fixes, device methods, storage methods)

use super::{GgmlDType, QStorage};
use crate::quantized::k_quants::GgmlType;
use crate::{backend::BackendDevice, rocm_backend::WrapErr};
use crate::{builder_arg as barg, Result, RocmDevice, RocmStorage};
use half::f16;
...
#[derive(Clone, Debug)]
struct PaddedDeviceMemory {
    inner: DeviceMemory<u8>,
    len: usize,
}
...
pub const GGML_HIP_MMV_X: usize = 32;
pub const GGML_HIP_MMV_Y: usize = 1;
pub const HIP_QUANTIZE_BLOCK_SIZE: usize = 256;
pub const HIP_DEQUANTIZE_BLOCK_SIZE: usize = 256;
...
fn quantize_q8_1(
    ...
    let num_blocks = ceil_div(kx_padded, HIP_QUANTIZE_BLOCK_SIZE);
    // TEAM-507: Use get_or_load_func_raw for runtime-compiled kernels
    let func = dev.get_or_load_func_raw("quantize_q8_1", quantized_stub::QUANTIZED.as_bytes())?;
...
fn dequantize_f32(
    ...
    let (kernel_name, is_k, block_dim, num_blocks) = match dtype {
        ...
        GgmlDType::Q5_0 => (
            ...
            false,
            HIP_DEQUANTIZE_BLOCK_SIZE,
            ceil_div(elem_count, 2 * HIP_DEQUANTIZE_BLOCK_SIZE),
        ),
        GgmlDType::Q5_1 => (
    ...
    // TEAM-507: Use get_or_load_func_raw for runtime-compiled kernels
    let func = dev.get_or_load_func_raw(kernel_name, quantized_stub::QUANTIZED.as_bytes())?;
    let dst = unsafe { dev.alloc::<f32>(elem_count)? };
    // See e.g.
    // https://github.com/ggerganov/llama.cpp/blob/cbbd1efa06f8c09f9dff58ff9d9af509cc4c152b/ggml-cuda.cu#L7270
    let grid_dim = Dim3 { x: num_blocks as u32, y: 1, z: 1 };
    let block_dim_dim3 = Dim3 { x: block_dim as u32, y: 1, z: 1 };

    if is_k {
        let mut kernel_params: Vec<*mut c_void> =
            vec![&data.inner as *const _ as *mut c_void, &dst as *const _ as *mut c_void];
        func.launch(grid_dim, block_dim_dim3, 0, None, &mut kernel_params).w()?;
    } else {
        let nb32 = match dtype {
            GgmlDType::Q5_0 | GgmlDType::Q5_1 => elem_count,
            _ => elem_count / 32,
        };
        let nb32_i32 = nb32 as i32;
        let mut kernel_params: Vec<*mut c_void> = vec![
            &data.inner as *const _ as *mut c_void,
            &dst as *const _ as *mut c_void,
            &nb32_i32 as *const i32 as *mut c_void,
        ];
        func.launch(grid_dim, block_dim_dim3, 0, None, &mut kernel_params).w()?;
    }
...
fn dequantize_f16(
    ...
    Ok(RocmStorage::wrap_rocm_slice_f16(dst, dev.clone()))
}
...
fn dequantize_mul_mat_vec(
    ...
    let kernel_name = match dtype {
        GgmlDType::Q4_0 => "dequantize_mul_mat_vec_q4_0_hip",
        GgmlDType::Q4_1 => "dequantize_mul_mat_vec_q4_1_hip",
        GgmlDType::Q5_0 => "dequantize_mul_mat_vec_q5_0_hip",
        GgmlDType::Q5_1 => "dequantize_mul_mat_vec_q5_1_hip",
        GgmlDType::Q8_0 => "dequantize_mul_mat_vec_q8_0_hip",
    ...
    let block_num_y = ceil_div(nrows, GGML_HIP_MMV_Y);
    let grid_dim = Dim3 { x: block_num_y as u32, y: 1, z: 1 };
    let block_dim = Dim3 { x: WARP_SIZE as u32, y: GGML_HIP_MMV_Y as u32, z: 1 };
    ...
    let mut kernel_params: Vec<*mut c_void> = vec![
        ...
        &dst as *const _ as *mut c_void,
        &ncols_i32 as *const i32 as *mut c_void,
        &nrows_i32 as *const i32 as *mut c_void,
    ];
    func.launch(grid_dim, block_dim, 0, None, &mut kernel_params).w()?;
    Ok(RocmStorage::wrap_rocm_slice(dst, dev.clone()))
}
...
fn mul_mat_vec_via_q8_1(
    ...
    let kernel_name = match dtype {
        ...
        GgmlDType::Q5_0 => "mul_mat_vec_q5_0_q8_1_hip",
        GgmlDType::Q5_1 => "mul_mat_vec_q5_1_q8_1_hip",
        GgmlDType::Q8_0 => "mul_mat_vec_q8_0_q8_1_hip",
        GgmlDType::Q2K => "mul_mat_vec_q2_K_q8_1_hip",
        GgmlDType::Q3K => "mul_mat_vec_q3_K_q8_1_hip",
        GgmlDType::Q4K => "mul_mat_vec_q4_K_q8_1_hip",
        GgmlDType::Q5K => "mul_mat_vec_q5_K_q8_1_hip",
        GgmlDType::Q6K => "mul_mat_vec_q6_K_q8_1_hip",
        _ => crate::bail!("unsupported dtype for quantized matmul {dtype:?}"),
    };
    ...
    let grid_dim = Dim3 { x: nblocks, y: 1, z: 1 };
    let block_dim = Dim3 { x: WARP_SIZE as u32, y: nwarps, z: 1 };
    ...
    let mut kernel_params: Vec<*mut c_void> = vec![
        &data.inner as *const _ as *mut c_void,
        &y_q8_1 as *const _ as *mut c_void,
        &dst as *const _ as *mut c_void,
        &ncols_i32 as *const i32 as *mut c_void,
        &nrows_i32 as *const i32 as *mut c_void,
        &ncols_padded_i32 as *const i32 as *mut c_void,
        &nrows_i32 as *const i32 as *mut c_void,
    ];
    func.launch(grid_dim, block_dim, 0, None, &mut kernel_params).w()?;
    Ok(RocmStorage::wrap_rocm_slice(dst, dev.clone()))
...
fn mul_mat_via_q8_1(
    ...
    let grid_dim =
        Dim3 { x: ceil_div(x_rows, mmq_y) as u32, y: ceil_div(y_cols, mmq_x) as u32, z: 1 };
    let block_dim = Dim3 { x: WARP_SIZE as u32, y: 4, z: 1 };

    let x_cols_i32 = x_cols as i32;
    let x_rows_i32 = x_rows as i32;
    let y_cols_i32 = y_cols as i32;
    let k_padded_i32 = k_padded as i32;
    let mut kernel_params: Vec<*mut c_void> = vec![
        &data.inner as *const _ as *mut c_void,
        &y_q8_1 as *const _ as *mut c_void,
        &dst as *const _ as *mut c_void,
        &x_cols_i32 as *const i32 as *mut c_void,
        &x_rows_i32 as *const i32 as *mut c_void,
        &y_cols_i32 as *const i32 as *mut c_void,
        &k_padded_i32 as *const i32 as *mut c_void,
        &x_rows_i32 as *const i32 as *mut c_void,
    ];
    func.launch(grid_dim, block_dim, 0, None, &mut kernel_params).w()?;
    Ok(RocmStorage::wrap_rocm_slice(dst, dev.clone()))
...
impl QRocmStorage {
    pub fn zeros(device: &RocmDevice, el_count: usize, dtype: GgmlDType) -> Result<Self> {
        ...
        Ok(QRocmStorage {
            data: PaddedDeviceMemory { inner, len: size_in_bytes },
            device: device.clone(),
            dtype,
        })
    }
    ...
    pub fn dequantize(&self, elem_count: usize) -> Result<RocmStorage> {
        ...
        self.device.storage_from_cpu_storage(&crate::CpuStorage::F32(out))
    }
    ...
    pub fn quantize(&mut self, src: &RocmStorage) -> Result<()> {
        ...
        let src = match &src.slice {
            crate::rocm_backend::RocmStorageSlice::F32(data) => self.device.memcpy_dtov(data)?,
        ...
        self.data = PaddedDeviceMemory { inner, len: data.len() };
        Ok(())
    }
    ...
    pub fn fwd(
        ...
        let max_bm = if FORCE_DMMV.load(std::sync::atomic::Ordering::Relaxed) { 1 } else { 8 };
...
impl QRocmStorage {
    fn dequantize_matmul_vec(
        ...
        let out = if FORCE_DMMV.load(std::sync::atomic::Ordering::Relaxed) {
            ...
            mul_mat_vec_via_q8_1(&self.data, &rhs, self.dtype, ncols, nrows, b_size, self.device())?
        };
    ...
    fn dequantize_matmul(
        ...
        let out = if FORCE_DMMV.load(std::sync::atomic::Ordering::Relaxed) {
            ...
            let storage = match layout.contiguous_offsets() {
                ...
                None => Err(crate::Error::RequiresContiguous { op: "quantized-matmul" }.bt())?,
            };
            mul_mat_via_q8_1(
                &self.data,
                &storage,
...
mod test {
    ...
    fn cuda_mmv_q8_1() -> Result<()> {
        ...
        let y = dev.memcpy_stod(&vs)?;
        let mut xs = QRocmStorage::zeros(&dev, ncols, GgmlDType::Q4_0)?;
        xs.quantize(&RocmStorage::wrap_rocm_slice(y.clone(), dev.clone()))?;
        ...
        let vs = cuda_storage.as_hip_slice::<f32>()?;
    ...
    fn cuda_mm_q8_1() -> Result<()> {
        ...
        let y = dev.memcpy_stod(&vs)?;
        let mut xs = QRocmStorage::zeros(&dev, ncols * 4, GgmlDType::Q4_0)?;
        xs.quantize(&RocmStorage::wrap_rocm_slice(y.clone(), dev.clone()))?;
    ...
    fn cuda_mm_q8_1_pad() -> Result<()> {
        ...
        let y = dev.memcpy_stod(&vs)?;
        let mut xs = QRocmStorage::zeros(&dev, ncols * x_rows, GgmlDType::Q4_0)?;
        xs.quantize(&RocmStorage::wrap_rocm_slice(y.clone(), dev.clone()))?;
        let cuda_storage = mul_mat_via_q8_1(
            &xs.data,
            &y.slice(..),
