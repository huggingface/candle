...
mod tests {
    ...
    fn test_flip_1d() -> Result<()> {
        ...
        assert_tensor_eq(&flipped, &expected)?;
        Ok(())
    }
    ...
    fn test_flip_backprop() -> Result<()> {
        ...
        let x = Tensor::ones((2, 2), DType::F64, device)?.make_var()?;
        ...
        assert_tensor_eq(&grad_x, &expected_grad)?;
