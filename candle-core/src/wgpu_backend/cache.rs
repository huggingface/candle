...
use super::{util::{Reference, ReferenceTrait, Storage, StorageOptional, ToU64}, WgpuDevice};
use super::{
    device::PipelineType,
...
impl BufferReference{
    pub fn new(size: u64, referenced_by_candle_storage : bool) -> Self {
        Self { size, cached_buffer_id : CachedBufferId::new(0,0),referenced_by_candle_storage, first_used : 0, last_used : if referenced_by_candle_storage {u32::MAX} else {0} }
    }

    pub fn new_with_storage(size: u64, cached_buffer_id: CachedBufferId, referenced_by_candle_storage : bool) -> Self {
        Self { size, cached_buffer_id, referenced_by_candle_storage, first_used : 0, last_used : if referenced_by_candle_storage {u32::MAX} else {0}}
    }
...
impl ModelCache {
        ...
        if check_bindgroups{
            self.bindgroups.retain_bindgroups(|bindgroup |
            {
                ...
                match &bindgroup.buffer.get_input() {
                    ...
                    BindgroupInputBase::Bindgroup2(v1, v2, _) =>  check_buffer(v1) && check_buffer(v2),
                    BindgroupInputBase::Bindgroup3(v1, v2, v3) =>  check_buffer(v1) && check_buffer(v2) && check_buffer(v3),
                };
    ...
    pub(crate) fn get_bind_group(
        ...
        fn check_buffer_reference(cache : &mut ModelCache, bindgroup_reference: &BindgroupReferenceFull, pipeline: PipelineType,){
            ...
            let check_buffer = |buffer_reference_id| {
                if let Some(buffer_reference) = cache.buffer_reference.get(buffer_reference_id){
                    if !buffer_reference.cached_buffer_id.is_valid(){
                        ...
                        if cache.buffers.get_buffer(&buffer_reference.cached_buffer_id).is_none(){
                            if let Some(buffer_reference) = cache.buffer_reference.get(buffer_reference_id){
                    ...
                    if let Some(val) = cache.buffer_reference.get_reference(buffer_reference_id.id()){
            ...
            match &bindgroup_reference.1 {
                ...
                BindgroupInputBase::Bindgroup1(v1, is_16) => BindgroupInputBase::Bindgroup1(check_buffer(v1), *is_16),
                BindgroupInputBase::Bindgroup2(v1, v2, is_16) => BindgroupInputBase::Bindgroup2(check_buffer(v1),check_buffer(v2), *is_16),
                BindgroupInputBase::Bindgroup3(v1, v2, v3) => BindgroupInputBase::Bindgroup3(check_buffer(v1),check_buffer(v2),check_buffer(v3)),
            };
        }
        ...
        fn get_storage(cache : &ModelCache, id: &BufferReferenceId) -> CachedBufferId {
            cache.buffer_reference.get(&id).unwrap().cached_buffer_id.clone()
        }
        ...
        fn get_buffer_referece_key(
            ...
            return BindgroupFullBase(dest_buffer, 
            match &bindgroup_reference.1 {
                BindgroupInputBase::Bindgroup0 => BindgroupInputBase::Bindgroup0,
                BindgroupInputBase::Bindgroup1(v1, is_16) => BindgroupInputBase::Bindgroup1(get_storage(cache, v1), *is_16),
                BindgroupInputBase::Bindgroup2(v1, v2, is_16) => BindgroupInputBase::Bindgroup2(get_storage(cache, v1),get_storage(cache, v2), *is_16),
                BindgroupInputBase::Bindgroup3(v1, v2, v3) => BindgroupInputBase::Bindgroup3(get_storage(cache, v1),get_storage(cache, v2),get_storage(cache, v3)),
            });
        }
        ...
        if dev.use_cache {
            ...
            if buf_dest_cached_id.is_valid(){
                ...
                if let Some(bg) = self.bindgroups.get_bindgroup_reference_by_description(&bindgroup_inputs).cloned() {
                ...
                let candidates_to_process = self
                    ...
                    .filter_map(|(id, bindgroup)| {
                        ...
                        if buf_dest_cached_id.is_valid(){
                            ...
                            if let Some(c_buf_dest) = self.buffers.get_buffer(cbuf_dest_id){
                                if c_buf_dest.buffer.size() >= required_size && c_buf_dest.is_free() && c_buf_dest.buffer.size() <= max_size{
                                    return Some((id, bindgroup, c_buf_dest.buffer.size()))
                                }
                            }
                        }   
        ...
        let dest_buffer_id;
        if buf_dest_reference.cached_buffer_id.is_valid(){  //this buffer reference already has a buffer connected,use this buffer
    ...
    fn create_bindgroup(&mut self, dev : &WgpuDevice, bindgroup_d : CachedBindgroupFull) -> CachedBindgroupId {
        ...
        self.bindgroups.bindgroups.add_mapping(bindgroup_d.1.clone(), id);
...
impl<TI : Ord, TV : Ord> Ord for OrderedIndex<TI, TV> {
    fn cmp(&self, other: &Self) -> std::cmp::Ordering {
        self.value.cmp(&other.value).then_with(|| self.index.cmp(&other.index))
    }
}
...
impl BufferCacheStorage{
    ...
    pub fn delete_buffer(&mut self, id : &CachedBufferId){
        ...
        if let Some(val) = value{
            if self.order.remove(&OrderedIndex::new(id.id(), val.buffer.size())){
...
impl BindgroupCacheStorage {
    ...
    fn retain_bindgroups(&mut self, mut keep : impl FnMut(&CachedBindgroup) -> bool){
        self.storage.retain_mut(|(id, bg)| {
            ...
            if !keep{
                ...
                self.bindgroups.remove_mapping(buf_reference_input_full.1.clone(), &id);
    ...
    fn get_bindgroup_reference_by_description(&self, bindgroup_d : &CachedBindgroupFull) -> Option<&CachedBindgroupId>{
    ...
    fn enumerate_bindgroup_by_description_input(&self, bindgroup_d : &CachedBindgroupInput) -> impl Iterator<Item=(CachedBindgroupId, &CachedBindgroup)>{
        self.get_bindgroup_reference_by_description_input(bindgroup_d).iter().filter_map(|c| 
            Some((c.clone(), self.get_bindgroup(c)?))
        )
    }
