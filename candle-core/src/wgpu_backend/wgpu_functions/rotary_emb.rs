...
pub fn queue_rotary_emb_i(
    ...
    let pipeline = queue.get_pipeline(Pipelines::RotaryEmb(
        dev.get_dtype(dtype)?,
        candle_wgpu_kernels::rotary_emb::Functions::RotaryEmbI,
    ));
...
pub fn queue_rotary_emb_c(
    ...
    debug_assert!(d % 2 == 0, "RotaryEmbC requires even head_dim (d)");
    ...
    // op_meta[0..7] = B, H, T, D, src_offset, cos_offset, sin_offset
    queue.add(b);
    queue.add(h);
    queue.add(t);
    queue.add(d);
    ...
    // CONSTV_0 = unbatched flag (0/1)
    queue.add_const(Constants::Constv0, unbatched);

    let pipeline = queue.get_pipeline(Pipelines::RotaryEmb(
        dev.get_dtype(dtype)?,
        candle_wgpu_kernels::rotary_emb::Functions::RotaryEmbC,
    ));
...
pub fn queue_rotary_emb_thd(
    ...
    let pipeline = queue.get_pipeline(Pipelines::RotaryEmb(
        dev.get_dtype(dtype)?,
        candle_wgpu_kernels::rotary_emb::Functions::RotaryEmbThd,
    ));
