...
impl<B: BackendStorage> Tensor<Bwd<B>> {
    /// Return all the nodes that lead to this value in a topologically sorted vec, the first
    /// elements having dependencies on the latter ones, e.g. the first element if any is the
    /// argument.
    /// This assumes that the op graph is a DAG.
    pub fn sorted_nodes(&self) -> Vec<Tensor<Bwd<B>>> {
        // The vec of sorted nodes is passed as an owned value rather than a mutable reference
        // to get around some lifetime limitations.
        fn walk<B: BackendStorage>(
            node: Tensor<Bwd<B>>,
            nodes: Vec<Tensor<Bwd<B>>>,
            already_seen: &mut HashMap<TensorId, bool>,
        ) -> (bool, Vec<Tensor<Bwd<B>>>) {
            ...
            let mut nodes = if node.is_variable() {
                ...
                match op {
                    ...
                    Op::Cat(args, _) => args.into_iter().fold(nodes, |nodes, arg| {
        ...
        let (_tg, mut nodes) = walk(self.clone(), vec![], &mut HashMap::new());
        nodes.reverse();
        nodes
    }

    pub fn backward(&self) -> Result<GradStore<Bwd<B>>> {
            ...
            if let Some(op) = node.op() {
                match op {
                    ...
                    Op::Conv2D {
                        ...
                        let out_padding = arg.dim(2)? - out_size;
                        let grad_arg =
                            grad.conv_transpose2d(&kernel, padding, out_padding, stride, dilation)?;
                        let sum_grad = grads.or_insert(&arg)?;
                        *sum_grad = sum_grad.add(&grad_arg)?;
                    ...
                    Op::ToDevice(arg) => {
                        let sum_grad = grads.or_insert(&arg)?;
                        // TODO: Within the world of Op tensors are already on the same device.
...
impl<B: BackendStorage> GradStore<Bwd<B>> {
    ...
    pub fn get_id(&self, id: TensorId) -> Option<&Tensor<Bwd<B>>> {
        self.0.get(&id)
    }

    /// Get the gradient tensor associated with the given tensor
    pub fn get(&self, tensor: &Tensor<Bwd<B>>) -> Option<&Tensor<Bwd<B>>> {
        self.0.get(&tensor.id())
    }

    /// Remove the gradient tensor associated with the given tensor, returning it if it exists
    pub fn remove(&mut self, tensor: &Tensor<Bwd<B>>) -> Option<Tensor<Bwd<B>>> {
    ...
    pub fn insert_id(&mut self, id: TensorId, grad: Tensor<Bwd<B>>) -> Option<Tensor<Bwd<B>>> {
        self.0.insert(id, grad)
    }

    /// Get the gradient tensor associated with the given tensor, or, if it does not exist,
    /// insert a tensor of zeroes, with the same shape and type as the given tensors and return it
    fn or_insert(&mut self, tensor: &Tensor<Bwd<B>>) -> Result<&mut Tensor<Bwd<B>>> {
