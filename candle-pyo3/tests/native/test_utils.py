...
from candle.utils import load_safetensors, save_gguf, load_gguf, save_safetensors, parse_absolute_shape
...
def test_shape_parsing():
    result = parse_absolute_shape((1, 2, 3))
    assert result == [1, 2, 3]

    result = parse_absolute_shape(1, 2, 3)
    assert result == [1, 2, 3]

    result = parse_absolute_shape(1, -1, element_count=6)
    assert result == [1, 6]

    result = parse_absolute_shape(-1, 1, element_count=6)
    assert result == [6, 1]

    result = parse_absolute_shape(1, 5, -1, element_count=25)
    assert result == [1, 5, 5]
