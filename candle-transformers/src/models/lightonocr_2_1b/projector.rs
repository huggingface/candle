...
impl Projector {
    pub fn new(hidden_size: usize, vb: VarBuilder) -> Result<Self> {
        ...
        let linear_1 = candle_nn::linear_no_bias(hidden_size, hidden_size, vb.pp("linear_1"))?;

        let linear_2 = candle_nn::linear_no_bias(hidden_size, hidden_size, vb.pp("linear_2"))?;
    ...
    pub fn forward(&self, x: &Tensor, ph: usize, pw: usize) -> Result<Tensor> {
        ...
        let x = x
            .reshape((1, hidden, ph / 2, 2, pw / 2, 2))?
            .permute((0, 2, 4, 1, 3, 5))?
