...
impl CausalSelfAttention {
    ...
    fn load(vb: VarBuilder, cfg: &Config) -> Result<Self> {
        ...
        let q_proj = bit_linear(size_in, size_q, vb.pp("q_proj"))?;
        let k_proj = bit_linear(size_in, size_kv, vb.pp("k_proj"))?;
        let v_proj = bit_linear(size_in, size_kv, vb.pp("v_proj"))?;
        let o_proj = bit_linear(size_q, size_in, vb.pp("o_proj"))?;
        let inner_attn_ln = RmsNorm::new(size_q, cfg.rms_norm_eps, vb.pp("inner_attn_ln"))?;
        Ok(Self {
            q_proj,
            k_proj,
            v_proj,
...
impl Mlp {
    ...
    fn load(vb: VarBuilder, cfg: &Config) -> Result<Self> {
        ...
        let c_fc1 = bit_linear(h_size, i_size, vb.pp("gate_proj"))?;
        let c_fc2 = bit_linear(h_size, i_size, vb.pp("up_proj"))?;
        let c_proj = bit_linear(i_size, h_size, vb.pp("down_proj"))?;
        let ffn_layernorm = RmsNorm::new(i_size, cfg.rms_norm_eps, vb.pp("ffn_layernorm"))?;
        Ok(Self {
            c_fc1,
            c_fc2,
            c_proj,
