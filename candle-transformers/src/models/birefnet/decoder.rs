...
impl Decoder {
    ...
    pub fn forward(&self, features: &[Tensor]) -> Result<Vec<Tensor>> {
        ...
        let p4 = if let (Some(gdt_convs), Some(gdt_attn)) =
            (&self.gdt_convs_4, &self.gdt_convs_attn_4)
        {
        ...
        let p3 = if let (Some(gdt_convs), Some(gdt_attn)) =
            (&self.gdt_convs_3, &self.gdt_convs_attn_3)
        {
        ...
        let p2 = if let (Some(gdt_convs), Some(gdt_attn)) =
            (&self.gdt_convs_2, &self.gdt_convs_attn_2)
        {
