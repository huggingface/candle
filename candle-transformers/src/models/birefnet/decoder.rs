...
impl Decoder {
    pub fn new(channels: &[usize], config: &Config, vb: VarBuilder) -> Result<Self> {
        ...
        let (ipt_blk5, ipt_blk4, ipt_blk3, ipt_blk2, ipt_blk1, ipt_cha_tuple) = if config.dec_ipt {
            ...
            (
                Some(SimpleConvs::new(split_factor * 3, out_ch_5, ic, vb.pp("ipt_blk5"))?),
        ...
        let decoder_block4 = BasicDecBlk::new(
            channels[0] + ipt_cha_tuple.0, channels[1], config.dec_att, use_bn, vb.pp("decoder_block4"),
        )?;
        let decoder_block3 = BasicDecBlk::new(
            channels[1] + ipt_cha_tuple.1, channels[2], config.dec_att, use_bn, vb.pp("decoder_block3"),
        )?;
        let decoder_block2 = BasicDecBlk::new(
            channels[2] + ipt_cha_tuple.2, channels[3], config.dec_att, use_bn, vb.pp("decoder_block2"),
        )?;
        let decoder_block1 = BasicDecBlk::new(
            channels[3] + ipt_cha_tuple.3, channels[3] / 2, config.dec_att, use_bn, vb.pp("decoder_block1"),
        )?;
        ...
        let conv_out1 = candle_nn::conv2d(
            channels[3] / 2 + ipt_cha_tuple.4, 1, 1, Default::default(), vb.pp("conv_out1.0"),
        )?;

        // Multi-scale supervision
        let (conv_ms_spvn_4, conv_ms_spvn_3, conv_ms_spvn_2) = if config.ms_supervision {
            (
                Some(candle_nn::conv2d(channels[1], 1, 1, Default::default(), vb.pp("conv_ms_spvn_4"))?),
                Some(candle_nn::conv2d(channels[2], 1, 1, Default::default(), vb.pp("conv_ms_spvn_3"))?),
                Some(candle_nn::conv2d(channels[3], 1, 1, Default::default(), vb.pp("conv_ms_spvn_2"))?),
            )
        } else {
            (None, None, None)
        };
        ...
        Ok(Self {
            config: config.clone(),
            ipt_blk5, ipt_blk4, ipt_blk3, ipt_blk2, ipt_blk1,
            decoder_block4, decoder_block3, decoder_block2, decoder_block1,
            lateral_block4, lateral_block3, lateral_block2,
            conv_out1,
    ...
    pub fn forward(&self, features: &[Tensor]) -> Result<Vec<Tensor>> {
        let (x, x1, x2, x3, x4) = (&features[0], &features[1], &features[2], &features[3], &features[4]);
        let mut outs = Vec::new();

        // Stage 4
        let x4 = if let Some(ipt_blk5) = &self.ipt_blk5 {
            let patches = if self.config.dec_ipt_split { self.get_patches_batch(x, x4)? } else { x.clone() };
        ...
        let _p3 = if let Some(ipt_blk4) = &self.ipt_blk4 {
            let patches = if self.config.dec_ipt_split { self.get_patches_batch(x, &_p3)? } else { x.clone() };
        ...
        let _p2 = if let Some(ipt_blk3) = &self.ipt_blk3 {
            let patches = if self.config.dec_ipt_split { self.get_patches_batch(x, &_p2)? } else { x.clone() };
        ...
        let _p1 = if let Some(ipt_blk2) = &self.ipt_blk2 {
            let patches = if self.config.dec_ipt_split { self.get_patches_batch(x, &_p1)? } else { x.clone() };
        ...
        outs.push(self.conv_out1.forward(&_p1)?);
        Ok(outs)
    }
}
