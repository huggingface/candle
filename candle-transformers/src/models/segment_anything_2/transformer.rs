//! Two way transformer used by the SAM 2 mask decoder.
//!
//! This matches the SAM 1 transformer except for the MLP inside each block, which SAM 2 builds
//! from the generic `MLP` helper and therefore names `mlp.layers.{0,1}` rather than
//! `mlp.lin{1,2}`.
...
impl Attention {
    ...
    fn separate_heads(&self, xs: &Tensor) -> Result<Tensor> {
        let (b, n, c) = xs.dims3()?;
        xs.reshape((b, n, self.num_heads, c / self.num_heads))?
            .transpose(1, 2)?
            .contiguous()
    }

    fn recombine_heads(&self, xs: &Tensor) -> Result<Tensor> {
        let (b, n_heads, n_tokens, c_per_head) = xs.dims4()?;
        xs.transpose(1, 2)?
