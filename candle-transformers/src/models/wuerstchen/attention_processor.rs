...
impl<B: BackendStorage> Attention<B> {
    ...
    fn batch_to_head_dim(&self, xs: &Tensor<B>) -> Result<Tensor<B>> {
    ...
    fn head_to_batch_dim(&self, xs: &Tensor<B>) -> Result<Tensor<B>> {
        let (b_size, seq_len, dim) = xs.dims3()?;
    ...
    fn get_attention_scores(&self, query: &Tensor<B>, key: &Tensor<B>) -> Result<Tensor<B>> {
        let attn_probs = (query.matmul(&key.t()?)? * self.scale)?;
        candle_nn::ops::softmax_last_dim(&attn_probs)
    }

    pub fn forward(&self, xs: &Tensor<B>, encoder_hidden_states: &Tensor<B>) -> Result<Tensor<B>> {
