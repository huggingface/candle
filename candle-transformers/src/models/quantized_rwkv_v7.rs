//! RWKV v7 model implementation with quantization support.
//!
//! RWKV is a linear attention model that combines the efficiency of RNNs
//! with the parallelizable training of Transformers. Version 7 builds on previous
...
//! - [RWKV v7 Release](https://huggingface.co/BlinkDL/rwkv-7-world)
//!
...
impl SelfAttention {
    fn new(layer_id: usize, cfg: &Config, vb: VarBuilder) -> Result<Self> {
        ...
        let x_r = vb
            .get((1, 1, cfg.hidden_size), "x_r")?
            .dequantize(vb.device())?;
        let x_w = vb
            .get((1, 1, cfg.hidden_size), "x_w")?
            .dequantize(vb.device())?;
        let x_k = vb
            .get((1, 1, cfg.hidden_size), "x_k")?
            .dequantize(vb.device())?;
        let x_v = vb
            .get((1, 1, cfg.hidden_size), "x_v")?
            .dequantize(vb.device())?;
        let x_a = vb
            .get((1, 1, cfg.hidden_size), "x_a")?
            .dequantize(vb.device())?;
        let x_g = vb
            .get((1, 1, cfg.hidden_size), "x_g")?
            .dequantize(vb.device())?;
        let r_k = vb
            .get((n_head, head_size), "r_k")?
            .dequantize(vb.device())?;
        let w0 = vb
        ...
        let k_k = vb
            .get((1, 1, cfg.hidden_size), "k_k")?
            .dequantize(vb.device())?;
        let k_a = vb
            .get((1, 1, cfg.hidden_size), "k_a")?
            .dequantize(vb.device())?;
